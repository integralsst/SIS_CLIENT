...
export const MATRIX_COLUMNS: MatrixColumnDefinition[] = [
  ...
  {
    key: "porcentajeCategoria",
    label: "% esperado categoría",
    width: 155,
    defaultVisible: true,
  },
  {
    key: "calificacionMinisterial",
    label: "Calif. ministerial esperada",
    width: 175,
    defaultVisible: true,
  },
  {
  ...
  {
    key: "documentoPeriodico",
    label: "Documento actual periódico",
    width: 185,
    defaultVisible: true,
  },
  {
...
export function formatPeriodicity(
  ...
  return `Cada ${validity.cantidad} ${
    UNIT_LABEL[validity.unidad]
  }`;
}

export function formatDailyTask(
  aspect: AspectCatalog
): string {
  ...
  return `${config.cantidadObjetivo} ${
    UNIT_LABEL[config.unidad]
  }`;
}
...
export function columnTotalWidth(
