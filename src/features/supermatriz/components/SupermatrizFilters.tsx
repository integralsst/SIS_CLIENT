...
interface Props {
  ...
  onChange: (patch: Partial<MatrixFilters>) => void;
}
...
export default function SupermatrizFilters({
  ...
  const standardCategories = useMemo(
    () =>
      catalogs.categoriasEstandar.filter(
        (item) =>
          !filters.cicloPhvaId ||
          item.cicloPhvaId === Number(filters.cicloPhvaId)
      ),
    [catalogs.categoriasEstandar, filters.cicloPhvaId]
  );
  ...
  const standards = useMemo(
    ...
    [catalogs.estandares, filters.categoriaEstandarId]
  );

  return (
    <section className="rounded-2xl border border-neutral-800/70 bg-[#111111] p-4 sm:p-5">
      <div className="mb-4 flex items-center gap-2 text-sm font-semibold text-white">
        <SlidersHorizontal size={17} className="text-cyan-400" />
        Filtros de la Supermatriz
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <AppSelect
          value={filters.versionSupermatrizId}
          onChange={(event) =>
            onChange({ versionSupermatrizId: event.target.value })
          }
        >
          <option value="">Todas las versiones</option>
          {catalogs.versiones.map((version) => (
        ...
        <AppSelect
          ...
          onChange={(event) =>
            onChange({
              estado: event.target.value as MatrixFilters["estado"],
            })
          }
        >
        ...
        >
          <option value="">Todos los ciclos PHVA</option>
        ...
        <AppSelect
          ...
          onChange={(event) =>
            onChange({
              categoriaEstandarId: event.target.value,
              estandarId: "",
            })
          }
        >
        ...
        <AppSelect
          ...
          onChange={(event) =>
            onChange({ estandarId: event.target.value })
          }
        >
          <option value="">Todos los estándares</option>
          {standards.map((standard) => (
            <option key={standard.id} value={standard.id}>
              {standard.codigo ? `${standard.codigo}. ` : ""}
              {standard.nombre}
            </option>
          ))}
        </AppSelect>
        ...
        <AppSelect
          ...
          onChange={(event) =>
            onChange({ procesoId: event.target.value })
          }
        >
        ...
        <AppSelect
          ...
          onChange={(event) =>
            onChange({ categoriaGestionId: event.target.value })
          }
        >
        ...
        <AppSelect
          ...
          onChange={(event) =>
            onChange({ grupoMinisterialId: event.target.value })
          }
        >
          <option value="">Todos los grupos 7/21/60</option>
